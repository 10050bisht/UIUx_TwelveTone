...
public class login { // Reuse the code and commented code is same code
	...
	private String getToasterMessage() {
		...
		WebElement toastElement = wait
				.until(ExpectedConditions.visibilityOfElementLocated(By.className("Toastify__toast-body")));
	...
	public void valid_login() {
		...
		WebElement toastElement = wait.until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[contains(text(), 'Login success')]")));
	...
	// driver.findElement(By.className("_submitBtn_1e7ib_96"));
	// submitBtn.click();
	//
	...
	// Assert.assertEquals(actualMessage.trim(), expectedMessage, "The toast message
