...
public class Billing_expected_Result {
	...
	// @Test
	// private void saveALLClientRowsToExcel() throws InterruptedException,
	// IOException {
	// Thread.sleep(5000); // Wait for the page to load
	//
	// // Locate rows with "Failed" status
	// List<WebElement> AllClientRows =
	// driver.findElements(By.xpath("//tr[contains(@class, 'MuiTableRow-root
	// css-2ygh3t')]"));
	//
	...
	// FileOutputStream("Billing_reports/BillingReport.xlsx")) {
	...
	// List<WebElement> totalInvoiceCells =
	// driver.findElements(By.xpath("//tr/td[6]"));
	//
	// // Locate all "Surcharge" cells
	// List<WebElement> surchargeCells =
	// driver.findElements(By.xpath("//tr/td[7]"));
	//
	...
	// List<WebElement> TotalDueCells =
	// driver.findElements(By.xpath("//tr/td[10]"));
	//
	// // Locate all "Total Paid" cells
	// List<WebElement> TotalPaidCells =
	// driver.findElements(By.xpath("//tr/td[11]"));
	//
	...
	// String invoiceText = totalInvoiceCells.get(i).getText().replace("$",
	// "").replace(",", "").trim();
	// String surchargeText = surchargeCells.get(i).getText().replace("$",
	// "").replace(",", "").trim();
	// String discountText = DiscountCells.get(i).getText().replace("$",
	// "").replace(",", "").trim();
	// String creditsText = CreditsCells.get(i).getText().replace("$",
	// "").replace(",", "").trim();
	// String dueText = TotalDueCells.get(i).getText().replace("$", "").replace(",",
	// "").trim();
	// String paidText = TotalPaidCells.get(i).getText().replace("$",
	// "").replace(",", "").trim();
	//
	...
	//// } catch (NumberFormatException e) {
	//// System.out.println("Non-numeric Surcharge value skipped: " +
	// surchargeText);
	//// }
	//
	//// try {
	...
	//// System.out.println("Invoice: " + invoiceText + ", Surcharge: " +
	// surchargeText);
	// }
	//
	...
	// totalRow.createCell(1).setCellValue("Total Surchage: " +
	// totalSurchargeAmount);
	// totalRow.createCell(2).setCellValue("Total Discount: " +
	// totalDiscountAmount);
	...
	// // Save the Excel file
	// try (FileOutputStream fileOut = new
	// FileOutputStream("Billing_reports/ExpectedAmount.xlsx")) {
	// workbook.write(fileOut);
	...
	public void saveAllClientDetailsToExcel() throws InterruptedException, IOException {
		...
		List<WebElement> allClientRows = driver
